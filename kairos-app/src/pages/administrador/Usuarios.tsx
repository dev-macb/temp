...
const Usuarios: React.FC = () => {
    ...
    const [usuarios, definirUsuarios] = useState<IUsuario[]>([]);
    const [administradores, definirAdministradores] = useState<IAdministradores[]>([]);

    const buscarUsuarios = async () => {
        try {
            const resultado = await ApiServidor.get('/admin/usuarios');
            console.log(resultado.data)
            definirUsuarios(resultado.data);
        } 
        catch (error) {
    ...
    const buscarAdministradores = async () => {
        try {
            const resultado = await ApiServidor.get('/admin/administradores');
                ...
                <Caixa.Vertical style={{ backgroundColor: 'white', borderRadius: '10px', padding: '20px', border: '1px solid #909090' }}>
                    <CaixaHorizontal>
                        <div>
                            ...
                            <p style={{fontSize: '2rem', color: '#007BFF'}}>{usuarios.length}</p>
                        </div>
            ...
            <Caixa.Vertical style={{ height: 'auto' }}>
                ...
                <Caixa.Vertical style={{ width: '100%' }}>
                    <Tabela2 
                        dados={usuarios}
                        nomeColunas={[
                            { chave: 'nome', textoCabecalho: 'Nome Completo'},
                            { chave: 'cpf', textoCabecalho: 'CPF'},
                            { chave: 'nascimento', textoCabecalho: 'Nascimento'},
                            { chave: 'sexo', textoCabecalho: 'Sexo'},
                            { chave: 'tipo', textoCabecalho: 'Tipo'},
                            { chave: 'acoes', textoCabecalho: 'Ações', elemento: (_, row) => (
